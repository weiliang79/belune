...
**Text on the accent colour** → `text-brand-fg`, never `text-white`. `--brand-fg` is computed per accent *and* per theme, and it happens to land on white in all four combinations today — but that's a property of the current palette, not a promise: dark+emerald only got there because `--brand` itself was deliberately darkened (see the `.dark[data-accent="emerald"]` comment in `index.css`) specifically so its foreground could stay white like the others. A future accent or a lighter brand color could need dark text again, and `text-brand-fg` is what stays correct automatically when that happens — a hardcoded `text-white` would need auditing everywhere it appears. `text-white` is only correct on a fixed surface such as the destructive button's red.
