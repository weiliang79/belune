          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-muted-foreground text-sm">
                Persistent layer cache reused across builds. Clearing forces the
                next build to start from scratch.
              </p>
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  <div className="text-muted-foreground">Build layers</div>
                  <div className="font-mono">
                    {buildCache ? formatBytes(Math.max(0, buildCache.build_cache_bytes)) : "—"}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Launch layers</div>
                  <div className="font-mono">
                    {buildCache ? formatBytes(Math.max(0, buildCache.launch_cache_bytes)) : "—"}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Total</div>
                  <div className="font-mono font-semibold">
                    {buildCache ? formatBytes(Math.max(0, buildCache.total_bytes)) : "—"}
                  </div>
                </div>
              </div>
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      The next build will re-download and rebuild all layers.
                      This can take significantly longer than a cached build.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
