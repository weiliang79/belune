...
<Callout type="warn">
A **Local** destination doesn't survive losing the host and is not disaster recovery on its own. Pair it with an S3-compatible destination (or the control-plane's own [remote storage](/docs/core/server/configurations#backups)) if you need to recover from a dead server.
</Callout>
...
See more on [Backing up application volumes](/docs/core/application/mounts#backing-up-volumes) or [Database backup & restore](/docs/core/database/backup-restore). 
