...
4. If the data volume is corrupt (rare), restore from a backup —
   see [platform backup & restore](/docs/core/server/configurations#backups).
...
[Certificates](/docs/core/server/certificates) for every status and failure. When the badge
tells you nothing:
