...
- **Caddy** — reverse proxy and automatic HTTPS for every domain on the host, including the dashboard's own. Its admin API on `127.0.0.1:2019` is how Belune programs routes and reads certificate status live — see [Certificates](/docs/core/server/certificates).
...
Belune keeps Caddy's routing table in sync by pushing config to its admin API whenever a domain is added, removed, or an app's container is recreated — this is the [reconcile step](/docs/core/server/configurations) you can also trigger manually from Server → Configurations.
...
2. **Build** — the builder you chose is used, or if you left it on auto one is picked in priority order: **Dockerfile** (if present) → **Cloud Native Buildpacks** → **Railpack**. See [Builders](/docs/core/application/builders).
...
- Databases are attached to their project network only — there is no public port by default. [External Access](/docs/core/database/overview) opens a database to the outside world through an on-demand SSH tunnel rather than a permanent published port.
...
| `caddylogs`                 | Caddy's access log, tailed by `belune` for [request logs](/docs/core/application/logs-metrics)               |
| `buildkitcache`             | BuildKit's layer cache, so rebuilds don't start cold                                        |

Application volume and file mounts get their own named volumes and host-side files respectively, outside this fixed set — see [Mounts](/docs/core/application/mounts).