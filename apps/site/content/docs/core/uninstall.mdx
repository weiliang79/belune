...
This dumps Postgres and the Caddy TLS data to a timestamped directory. Copy it off the host (`scp`, an S3-compatible destination, whatever you already use) before continuing. See [Configurations](/docs/core/server/configurations) if you'd rather trigger a backup from the dashboard first.
...
- **DNS records** pointing at this host — Belune doesn't manage DNS, so any `A` records you created during [install](/docs/core/install) will keep resolving here until you remove them yourself.
