...
<Callout type="warn">
Saving new variables doesn't touch the running container — it takes effect on the next deploy or reload; see [Deployments](/docs/core/application/deployments).
</Callout>
...
them runs. See [Next.js](/docs/core/application/frameworks/next) and [Vite](/docs/core/application/frameworks/vite) for practical guidance.
...
To see or copy it, use the **reveal** (eye) action on the row, which fetches the value on demand. Every reveal is recorded in the [audit log](/docs/core/server/audit-log), so handing back plaintext the UI otherwise hides leaves a trace. This works the same way as [file-mount secrets](/docs/core/application/mounts#secrets).
