...
This isn't a substitute for host access — see [Host shell](/docs/core/server/configurations#host-shell) if you need a shell on the server itself rather than inside one app's container, a materially larger capability with its own opt-in and audit trail.
