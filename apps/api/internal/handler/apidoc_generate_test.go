// apidoc_generate_test.go generates apps/site/content/docs/api-reference/*.mdx
...
// already uses to read apps/web — three levels up reaches apps/, same depth.
const apidocOutDir = "../../../site/content/docs/api-reference"
...
func writeAPIReferenceMDX(t *testing.T, routes []apidocRoute) {
	...
	pages := []string{"index"}
	written := map[string]bool{"index.mdx": true, "meta.json": true}
	for _, o := range apidocDomainOrder {
		d := bySlug[o.slug]
		if len(d.Rows) == 0 {
	...
	// index.mdx is hand-written and never touched.
